    ...
    function(_, Backbone, units, discrete_states, sensor_specific_states, threshold_states) {
        ...
        var model = Backbone.Model.extend({
			...
			st = st.substr(0, st.length - 1);
				...
				if(typeof(sensor_specific_states['sensor_specific_' + this.get('type_number') + '_' + bitpos]) != 'undefined'){
                   ...
                   //ks_lz_20170714  中英文混用 ++>>
                    if (!st) {
                    		st = sensor_specific_states["Normal"]; //st = "No state defined";
                    }
                   //ks_lz_20170714  中英文混用 ++>>
		    else{
			st = st.substr(0, st.length - 1);
            ...
            discrete_state: function() {
                return this.get('discrete_state');
            },
